// 몬스터 생성 함수
function createMonster(name, hp, attack, defense, expReward, dropRate, gold) {
  return {
    name: name,
    hp: hp,
    attack: attack,
...
// 몬스터 생성
const Goblin = createMonster("고블린", 20, 8, 2, 15, 0.5, 3);
const Dragon = createMonster("드래곤", 80, 40, 10, 70, 0.9, 50);
const Gorilla = createMonster("고릴라", 50, 20, 5, 30, 0.7, 30);
const Cow = createMonster("황소", 30, 12, 3, 20, 0.5, 10);
...
// 캐릭터 생성 함수
function createCharacter(job, gender) {
  return {
    job: job,
    gender: gender,
    hp: baseStats[job].hp + genderModifiers[gender].hp,
    attack: baseStats[job].attack + genderModifiers[gender].attack,
    defense: baseStats[job].defense + genderModifiers[gender].defense,
  };
}

// 캐릭터 생성
const mWarrior = createCharacter("전사", "남자");
const wWarrior = createCharacter("전사", "여자");
const mWizard = createCharacter("마법사", "남자");
const wWizard = createCharacter("마법사", "여자");
...
start.addEventListener("click", () => {
  ...
  switch (Cjob) {
    case "전사":
      switch (Cgender) {
        case "남자":
          ch.append(`HP: ${mWarrior.hp}`);
          break;
        case "여자":
          ch.append(`HP: ${wWarrior.hp}`);
          break;
      }
      break;
    ...
    case "마법사":
      switch (Cgender) {
        case "남자":
          ch.append(`HP: ${mWizard.hp}`);
          break;
        case "여자":
          ch.append(`HP: ${wWizard.hp}`);
          break;
      }
      break;
  }
  ...
  switch (Cjob) {
    case "전사":
      switch (Cgender) {
        case "남자":
          ca.append(`공격력: ${mWarrior.attack}`);
          break;
        case "여자":
          ca.append(`공격력: ${wWarrior.attack}`);
          break;
      }
      break;
    case "마법사":
      switch (Cgender) {
        case "남자":
          ca.append(`공격력: ${mWizard.attack}`);
          break;
        case "여자":
          ca.append(`공격력: ${wWizard.attack}`);
          break;
      }
      break;
  }
  ...
  switch (Cjob) {
    case "전사":
      switch (Cgender) {
        case "남자":
          cd.append(`방어력: ${mWarrior.defense}`);
          break;
        case "여자":
          cd.append(`방어력: ${wWarrior.defense}`);
          break;
      }
      break;
    case "마법사":
      switch (Cgender) {
        case "남자":
          cd.append(`방어력: ${mWizard.defense}`);
          break;
        case "여자":
          cd.append(`방어력: ${wWizard.defense}`);
          break;
      }
      break;
  }
  ...
  switch (Cjob) {
    case "전사":
      switch (Cgender) {
        case "남자":
          img.setAttribute("src", "../images/남자_전사.png");
          break;
        case "여자":
          img.setAttribute("src", "../images/여자_전사.png");
          break;
      }
      break;
    ...
    case "마법사":
      switch (Cgender) {
        case "남자":
          img.setAttribute("src", "../images/남자_마법사.png");
          break;
        case "여자":
          img.setAttribute("src", "../images/여자_마법사.png");
          break;
      }
      break;
  }
  ...
  div.style.backgroundImage =
    'url("https://img.freepik.com/free-vector/glowing-background-with-bursting-rays_1017-32167.jpg")';
  ...
  if (!check.contains(divC)) {
    ...
    switch (mon.name) {
      case "고블린":
        monimg.setAttribute(
          "src",
          "https://cdn.imweb.me/thumbnail/20201010/7a695c37e004f.png"
        );
        monimg.classList.add("goblin");
        break;
      case "황소":
        monimg.setAttribute(
          "src",
          "https://i.namu.wiki/i/mWFscSnzck6WSRqiYpsu_C8lkvKPbdXtmAx1est6SM3gOWpkWkJjzX2HuwpcRbed0GksmfonHJ0rVu-Gka4QZQ.webp"
        );
        monimg.classList.add("cow");
        break;
      case "고릴라":
        monimg.setAttribute("src", "../images/고릴라.png");
        monimg.classList.add("gorilla");
        break;
      case "드래곤":
        monimg.setAttribute("src", "../images/드래곤.png");
        monimg.classList.add("dragon");
        break;
    }
